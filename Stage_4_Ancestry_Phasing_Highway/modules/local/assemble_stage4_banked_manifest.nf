process ASSEMBLE_STAGE4_MANIFEST {

    label 'process_low'
    container 'genvar-core:2.1.0'
    stageInMode 'copy'

    publishDir "${params.stage4_outdir}", mode: 'copy', overwrite: true, pattern: 'samples_*_stage4.yaml'

    input:
    path manifest_fragments

    output:
    path 'samples_*_stage4.yaml', emit: stage4_manifest
...
Path(f'samples_{safe_id}_stage4.yaml').write_text(content, encoding='utf-8')
PYEOF
    """
}
